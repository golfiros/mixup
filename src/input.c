...
void print_input(FILE *fp, va_list *ap) {
  ...
  fprintf(fp,
          ...
          input, input->port[0], input->port[1], input->gain, input->balance);
}

static inline void input_update_vol(struct input *input) {
...
RPC_DEFN(input_delete, (struct input *, input)) {
  struct data *data = rpc_data();
  core_cbk(data->core, impl_input_delete, input);
  rpc_relay("input_delete", ((void *)input));
  rpc_return();
}
...
RPC_DEFN(input_set_port, (struct input *, input), (long, idx), (char *, path)) {
  struct data *data = rpc_data();
...
CBK_DEFN(impl_input_update_vol, (struct input *, input)) {
  input_update_vol(input);
}
RPC_DEFN(input_set_gain, (struct input *, input), (double, gain)) {
  struct data *data = rpc_data();
  input->gain = fmin(fmax(gain, INPUT_MIN_GAIN), INPUT_MAX_GAIN);
  core_cbk(data->core, impl_input_update_vol, input);
  rpc_relay("input_set_gain", ((void *)input), input->gain);
  if (gain != input->gain)
    rpc_return(input->gain);
  else
    rpc_return();
}
RPC_DEFN(input_set_balance, (struct input *, input), (double, balance)) {
  struct data *data = rpc_data();
  input->balance = fmax(-MAX_BAL, fmin(MAX_BAL, balance));
  core_cbk(data->core, impl_input_update_vol, input);
  rpc_relay("bus_set_balance", ((void *)input), input->balance);
